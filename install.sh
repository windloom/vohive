...
REPO="${VOHIVE_RELEASE_REPO:-windloom/vohive}"
RAW_BRANCH="${VOHIVE_RELEASE_BRANCH:-main}"
...
install_mcc_mnc_table() {
  ...
  if [ -f "${DATA_DIR}/mcc-mnc-table.json" ] && [ "${FORCE}" != "1" ]; then
    log "已存在 MCC/MNC 运营商表，跳过: ${DATA_DIR}/mcc-mnc-table.json"
    return 0
  fi

  mcc_url="${VOHIVE_MCC_MNC_URL:-https://raw.githubusercontent.com/${REPO}/${RAW_BRANCH}/assets/mcc-mnc-table.json}"
  mcc_tmp="${TMP_DIR}/mcc-mnc-table.json"
  log "正在安装 MCC/MNC 运营商表: ${mcc_url}"
  if download_to "${mcc_url}" "${mcc_tmp}"; then
    run_root install -m 0644 "${mcc_tmp}" "${DATA_DIR}/mcc-mnc-table.json"
  else
    log "MCC/MNC 运营商表下载失败，继续安装二进制"
  fi
}
